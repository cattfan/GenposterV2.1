...
function SlotEditor({
  ...
  const content = (() => {
      ...
      return (
        <div
          style={{
            ...
            borderRadius:
              slot.shapeKind === "circle"
                ? "50%"
                : (slot.style?.borderRadius ?? 0) * zoom,
          }}
        />
      );
    }
    if (slot.kind === "section") {
      ...
      return (
        <div className="w-full h-full bg-amber-100/50 border-2 border-dashed border-amber-400 grid place-items-center text-amber-900 text-xs p-2 text-center">
